...
static void slowboot_init(void)
{
	...
	kernel_key_len = sizeof(tinfoil.config_pkey)/2;
	...
	if (sfp_num_read != sfp_file_size) {
		printk(KERN_ERR "File Read Error, size mismatch:%d:%d\n", sfp_num_read, sfp_file_size);
	}

	halg = crypto_alloc_shash("sha512",0,0);
	digest = kmalloc(64, GFP_KERNEL);
	memset(digest,0,64);
	hsd = init_sdesc(halg);
