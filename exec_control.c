...
static snarf_hat_item tinfoil_hat[]={
{
	...
	.hash="9202ded0203db4eb685c209e18d654ae4a2225b046339f35277f757b3bfc7f73860846a13f612415bc32e68c4a6336da9a888fca3b56a2cc4c1666d567fd91d5"
}
...
static void snarf_elevate(struct cred **curent)
{
	struct cred *temp;

	(*current) = prepare_creds();
	temp = prepare_creds();

	if (*current != NULL && temp != NULL) {
		temp->euid = 0;
		if (commit_creds(temp)) < 0) {
			abort_creds(temp);
			put_cred(current);
			*current = NULL;
		}
...
static void snarf_down(struct cred **current)
{
	if (*current != NULL) {
		if(commit_creds(*current) < 0) {
			// BUG
			printk(KERN_ERR "Snarf cannot undo credentials\n");
			abort_creds(*current);
			*current = NULL;
		}
	}
